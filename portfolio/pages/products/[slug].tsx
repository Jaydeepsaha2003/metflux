...
import { ArrowRight, Home, ChevronRight, Zap, Shield, Cpu, Wrench, Target, Settings, CheckCircle, Star, Award, Truck, Power, Layers } from "lucide-react";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
const ProductDetail = () => {
  ...
  const allProducts = [...globalProductsData.products, ...transformerProductsData.products];
  const product: Product | undefined = allProducts.find(p => p.slug === slug);
  ...
  const getProductIcon = (productId: string) => {
    const iconMap: Record<string, React.ReactNode> = {
      ...
      'lt-ct': <Zap className="w-6 h-6" />,
      'lt-pt': <Power className="w-6 h-6" />,
      'mv-ct': <Shield className="w-6 h-6" />,
      'mv-pt': <Layers className="w-6 h-6" />,
      'rvt-earthing-transformer': <Settings className="w-6 h-6" />,
      'outdoor-transformer': <Truck className="w-6 h-6" />,
      'control-transformer': <Wrench className="w-6 h-6" />
    };
...
{/* Product Overview Section */}
          ...
          {product.specifications && (
            <div className="fade-in-element mt-24">
              <div className="text-center mb-12">
                <h2 className="text-3xl font-display font-bold text-gray-900 mb-4">Technical Specifications</h2>
                <p className="text-xl text-gray-600 max-w-3xl mx-auto">
                  Detailed technical specifications for {product.title}.
                </p>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {Object.entries(product.specifications).map(([key, value]) => (
                    <div key={key} className="border-b border-gray-200 pb-4">
                      ...
                      <dd className="text-lg text-gray-900 font-medium">{value as string}</dd>
                    </div>
                  ))}
                </div>
              </div>
