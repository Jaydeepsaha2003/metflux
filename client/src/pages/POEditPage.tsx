  ...
  return (
    <div className="space-y-4">
      ...
      {item && item.status === 'ACTIVE' && (item.pcsProduced ?? 0) === 0 && (
        <>
          {/* Header strip with inline-expandable SO header edit (SO#, customer,
              dates, notes). Server blocks edits the moment any line on this SO
              has production logged. */}
          <SOHeaderEditor item={item} />
...
/* ---------- SO header editor ----------
   Sits above the item editor. Read-only strip by default — click the pencil
...
const SOHeaderEditor = ({ item }: { item: Item }) => {
  const queryClient = useQueryClient();
  const [open, setOpen] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Fetch the full PO header lazily — only after the user expands the editor.
  // Saves a round-trip for the (common) case where they just want to edit the
  // line item and skip the SO header entirely.
  const { data: po } = useQuery({
    queryKey: ['po-header', item.poOrderId],
    queryFn: () => api<POHeader>(`/po-orders/${item.poOrderId}`),
    enabled: open,
  });

  // Customers list for the dropdown — same lazy pattern.
  ...
  // Form state, hydrated when the PO data arrives.
  const [poNumber,     setPoNumber]     = useState('');
  const [customerId,   setCustomerId]   = useState('');
  const [orderDate,    setOrderDate]    = useState('');
  const [deliveryDate, setDeliveryDate] = useState('');
  const [notes,        setNotes]        = useState('');
  // Hydrate the form once the PO data comes in — keyed by the PO id so a
  // re-fetch reloads the form values too.
  useEffect(() => {
    if (!po) return;
  ...
  const save = useMutation({
    mutationFn: () => api(`/po-orders/${item.poOrderId}`, {
      method: 'PATCH',
      json: {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['po-items'] });
      queryClient.invalidateQueries({ queryKey: ['po-item', item.id] });
      queryClient.invalidateQueries({ queryKey: ['po-header', item.poOrderId] });
      setOpen(false);
      setError(null);
    },
  ...
  return (
    <section className="card p-3 space-y-3">
      {/* Read-only strip — always visible */}
      <div className="flex flex-wrap items-center gap-x-6 gap-y-1 text-sm">
        ...
        >
          {open
            ...
            : <><Pencil className="h-3.5 w-3.5" /> Edit SO header</>}
        </button>
      ...
      {open && (
        <div className="border-t border-slate-200 pt-3 space-y-3">
          {!po && <div className="text-xs text-slate-400">Loading SO header…</div>}
          {po && (
            <>
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  type="button"
                  onClick={() => save.mutate()}
                  disabled={save.isPending || !poNumber.trim() || !customerId || !orderDate || !deliveryDate}
