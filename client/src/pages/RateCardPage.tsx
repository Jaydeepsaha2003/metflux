...
export const RateCardPage = () => {
  ...
  const setCustomerId = (id: string) => setParams(id ? { customerId: id } : {});
  ...
  return (
    <div className="customer-workspace customer-rates max-w-full space-y-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button onClick={() => { setDraft({ ...emptyDraft(), customerId }); setError(null); }} className="btn-primary justify-center">
      ...
      {/* The card itself */}
      {isError ? <div className="card p-5" role="alert">Could not load rates. <button className="underline" onClick={() => refetch()}>Try again</button></div> : isLoading && !data ? (
        ...
        <div className="space-y-3">
          {groups.map((g) => (
            <div key={g.id} className="overflow-hidden rounded-xl border border-slate-200/70 bg-white shadow-e1">
              ...
              <table className="customer-rate-table w-full text-[13px] font-semibold">
                ...
                <tbody>
                  {g.rates.map((r) => (
                    <tr key={r.id} className="border-t border-slate-100 transition-colors hover:bg-brand-50/40">
                      <td data-label="Grade" className="px-3 py-2">{r.grade}</td>
                      <td data-label="Core type" className="px-3 py-2">
                        <span className={cn(
                      ...
                      <td data-label="Rate" className="px-3 py-2 text-right font-num tabular-nums text-brand-700">
                        {money(Number(r.rateValue))}<span className="text-slate-400">{basisLabel(r.rateBasis)}</span>
                      </td>
                      <td data-label="Note" className="px-3 py-2 font-normal text-slate-500">{r.notes || '—'}</td>
